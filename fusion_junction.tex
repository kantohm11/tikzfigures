...
\begin{tikzpicture}[thick,scale=1.7]
  ...
  \draw[postaction={on each segment={mid arrow}},color=TopOp1] (0,0) --node[midway,anchor=east,black] {$a$}  ++(\sep,.5) coordinate(m);
  \draw[postaction={on each segment={mid arrow}},color=TopOp2] (2*\sep,0) --node[midway,anchor=west,black] {$b$}  (m);
  \draw[arrow at={.7}{},color=TopOp3] (m) --node[midway,anchor=west] {$c$} ++(0,.5);
  \node[circle,fill,inner sep=0,minimum size=1.5mm] at (\sep,.5){};
\end{tikzpicture}
\end{document}
